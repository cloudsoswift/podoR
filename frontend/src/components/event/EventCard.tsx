  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        <p className="text-sm text-gray-500">{event.hostNickname}</p>
        <p className="text-sm text-gray-400">{formatDate(event.eventDate)}</p>
      </div>

      <div className="shrink-0 self-center">
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${streamStatusStyle[event.streamStatus] ?? "bg-gray-100 text-gray-500"}`}>
          {streamStatusLabel[event.streamStatus] ?? event.streamStatus}
        </span>
      </div>
    </Link>
  );
